...
const AD_TRACKER_KEYWORDS = /\b(ads?|advertis|adx|adserv|adtrack|analytic[s]?|beacon[s]?|click[s]?|consent|cookie[s]?|counter|fingerprint|pixel|popup|retarget|tagmanager|telemetry|track(?:er|ing)?)\b/i;
...
export function isAdOrTracker(host: string): boolean {
  ...
  if (AD_TRACKER_SUFFIXES.some((suffix) => normalized === suffix || normalized.endsWith(`.${suffix}`))) {
    return true;
  }
  // Match ad/tracking keywords in subdomain labels (skip last 2 labels = base domain)
  const labels = normalized.split(".");
  if (labels.length >= 3) {
    const subdomain = labels.slice(0, -2).join(".");
    if (AD_TRACKER_KEYWORDS.test(subdomain)) {
      return true;
    }
  }
  return false;
